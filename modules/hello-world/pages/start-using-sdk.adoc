...
In a production scenario, you may wish to set up users with more granular access roles. 
...
Couchbase RBAC is fully described in the xref:{version-server}@server:manage:manage-security/manage-users-and-roles.adoc[Manage Users, Groups, and Roles] page.
--
====
...
NOTE: We are working with the xref:{version-server}@server:manage:manage-settings/install-sample-buckets.adoc[travel-sample] data bucket. 
...
xref:howtos:kv-operations.adoc[Document Operations] operations, such as storing and retrieving documents, can be done using `Collection.Upsert()` and `Collection.Get()`.
