...
Updated sub document operations to automatically, transparently, reorder if an xattr operation is listed.
This will automatically reorder back to the order that the user defined when returning.
...
Return a FlushNotEnabled error if bucket flush not enabled.
* https://issues.couchbase.com/browse/GOCBC-719[GOCBC-719]:
Consistently create tracing spans for all HTTP requests.
* https://issues.couchbase.com/browse/GOCBC-728[GOCBC-728]:
Add cluster level Ping operation.
* https://issues.couchbase.com/browse/GOCBC-807[GOCBC-807]:
Update best effort retry strategy to use an exponential backoff calculator.
* https://issues.couchbase.com/browse/GOCBC-820[GOCBC-820]:
Remove context.Context from search index manager operations.
