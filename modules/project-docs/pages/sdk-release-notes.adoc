...
[source,console]
----
$ go get github.com/couchbase/gocb/v2@v2.1.7
----
...
Add uncommitted support for `FlexIndex` to `QueryOptions`.
* https://issues.couchbase.com/browse/GOCBC-942[GOCBC-942]:
Add uncommitted support for `Scope` level queries.
* https://issues.couchbase.com/browse/GOCBC-944[GOCBC-944]:
Add uncommitted support for `Scope` level analytics queries.
* https://issues.couchbase.com/browse/GOCBC-944[GOCBC-944]:
Add uncommitted support for `User` collections level RBAC.
...
Add missing `Terms`, `DateRanges`, and `NumericRanges` fields to `SearchFacetResult`.
