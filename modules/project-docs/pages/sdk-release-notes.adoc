...
Version {sdk_dot_minor} of the Go SDK implements the {sdk_api} xref:compatibility.adoc#api-version[SDK API].
...
[v2.13.0]
=== Version 2.13.0 (?? October 2026)

Version 2.13.0 is the first release in the Go SDK 2.13 series.

https://pkg.go.dev/github.com/couchbase/gocb/v2@v2.13.0?tab=doc[API Documentation]
...
== Go SDK 2.12 Releases

=== Version 2.12.5 (2 September 2026)

Version 2.12.5 is a maintenance release for the Go SDK 2.12.

https://pkg.go.dev/github.com/couchbase/gocb/v2@v2.12.5?tab=doc[API Documentation]

==== New Features and Behavioral Changes

* https://jira.issues.couchbase.com/browse/GOCBC-1831[GOCBC-1831]: Logs related to HTTP operations now always include a unique request ID, which was only present for some HTTP operations previously.

==== Fixed issues

* https://jira.issues.couchbase.com/browse/GOCBC-1861[GOCBC-1861]: Fixed an issue where a negative `Initial` in `IncrementOptions`/`DecrementOptions` was failing with `ErrInvalidArgument` in `couchbase2` mode. This is now interpreted as absence of initial value (i.e. If the document does not exist, the operation will fail with `ErrDocumentNotFound`), consistent with the behavior when using the `couchbase(s)` scheme.
* https://jira.issues.couchbase.com/browse/GOCBC-1855[GOCBC-1855]: Resolved a `WaitGroup`-related panic that could happen when `Cluster.Close` is called while operations are ongoing concurrently.
* https://jira.issues.couchbase.com/browse/GOCBC-1862[GOCBC-1862]: Resolved an issue in `couchbase2` mode where a relevant gRPC status detail block was ignored if it was not the first detail block in the response. This meant that generic errors could be returned instead of a more specific error that was appropriate for the gRPC status.
* https://jira.issues.couchbase.com/browse/GOCBC-1842[GOCBC-1842]: Fixed an issue where config attributes that we report in application telemetry and dispatch-level spans (Node UUID, canonical address) might not be propagated to a KV client if it is still dialing when the first cluster config is seen. This could result in the attributes missing from the application telemetry entries or dispatch spans, and in "failed to split host port" debug-level log lines.

=== Version 2.12.4 (16 June 2026)

Version 2.12.4 is a maintenance release for the Go SDK 2.12.

https://pkg.go.dev/github.com/couchbase/gocb/v2@v2.12.4?tab=doc[API Documentation]

==== Fixed issues

* https://jira.issues.couchbase.com/browse/GOCBC-1824[GOCBC-1824]: Resolved a data race in `Collection.Scan` when `ScanOptions.Concurrency` is set to a value greater than 1.
* https://jira.issues.couchbase.com/browse/GOCBC-1816[GOCBC-1816]: Resolved a data race during when `Cluster.Close` is called while operations are ongoing, due to how the meter was accessed internally.
* https://jira.issues.couchbase.com/browse/GOCBC-1816[GOCBC-1821]: Fixed the value of `ScanTermMaximum`, which had been encoded incorrectly. This could lead to documents missing from a scan result when document keys include nonstandard unicode characters. `ScanTermMaximum` is now equivalent to `utf8.MaxRune`.
* https://jira.issues.couchbase.com/browse/GOCBC-1828[GOCBC-1828]: Fixed an issue with `Collection.Scan` where any fatal errors from initial vBucket scans, could either be returned immediately, or via `ScanResult.Err` depending on goroutine scheduling. Now they are consistently returned immediately by `Collection.Scan`.
* https://jira.issues.couchbase.com/browse/GOCBC-1835[GOCBC-1835]: Fixed an issue when an already cancelled `context.Context` is provided to an operation where the operation may still succeed. It is now guaranteed that the operation will fail with `ErrRequestCanceled`.

=== Version 2.12.3 (20 May 2026)

Version 2.12.3 is a maintenance release for the Go SDK 2.12.

https://pkg.go.dev/github.com/couchbase/gocb/v2@v2.12.3?tab=doc[API Documentation]

Tracing and metrics with the `couchbase2` scheme should be considered stability level *volatile* at this time.

==== New Features and Behavioral Changes
* https://jira.issues.couchbase.com/browse/GOCBC-1815[GOCBC-1815]: 
When `snappy.Decode` fails a warning was emitted, but no information included on what went wrong.
This warning is now updated to include the command, opaque, status and, datatype.
(If redaction is enabled then  the key if redacted.)

==== Fixed Issues
* https://jira.issues.couchbase.com/browse/GOCBC-1812[GOCBC-1812]:
Fixed an issue where `WaitUntilReady` can fail with `ErrRequestCanceled` during the initial network resolution when the seed endpoints don't match either the 'default' or 'external' network's endpoints. 
In that case, existing connections are dropped, cancelling the in-progress bootstrap, but this does not indicate an error.


=== Version 2.12.2 (23 April 2026)

Version 2.12.2 is a maintenance release for the Go SDK 2.12.

https://pkg.go.dev/github.com/couchbase/gocb/v2@v2.12.2?tab=doc[API Documentation]

Tracing and metrics with the `couchbase2` scheme should be considered stability level *volatile* at this time.

==== Fixed Issues

* https://jira.issues.couchbase.com/browse/GOCBC-1803[GOCBC-1803]:
...
* https://jira.issues.couchbase.com/browse/GOCBC-1805[GOCBC-1805]:
Updated to the latest version of the `couchbase2` protocol, which resolves an issue where the latest `gocb` and `goprotostellar` versions were incompatible.
* https://jira.issues.couchbase.com/browse/GOCBC-1807[GOCBC-1807]:
Fixed an issue where the internal TLS configuration is not preserved when using `SetAuthenticator`, which resulted in subsequent connections attempting to connect to TLS ports without TLS enabled, and failing.
* https://jira.issues.couchbase.com/browse/GOCBC-1808[GOCBC-1808]:
Resolved an issue with `SetAuthenticator` where the references to existing KV connections were being lost. Existing connections were still active, but orphaned.
This resulted in failing operations until new connections were established.
* https://jira.issues.couchbase.com/browse/GOCBC-1809[GOCBC-1809]:
Fixed a deadlock that happens when `SetAuthenticator` is called with a `JWTAuthenticator` and authentication fails.
Resolved a panic that could occur when `SetAuthenticator` is called with a `JWTAuthenticator` and the SDK is in the process of dialing a KV connection.


=== Version 2.12.1 (23 March 2026)

Version 2.12.1 is a maintenance release for the Go SDK 2.12.

https://pkg.go.dev/github.com/couchbase/gocb/v2@v2.12.1?tab=doc[API Documentation]

Tracing and metrics with the `couchbase2` scheme should be considered stability level *volatile* at this time.

==== Fixed issues
* https://jira.issues.couchbase.com/browse/GOCBC-1796[GOCBC-1796]: Fixed a rare "send on closed channel" panic that could occur during a transactions `BulkGet` operation when documents were being fetched concurrently.
* https://jira.issues.couchbase.com/browse/GOCBC-1799[GOCBC-1799]: Fixed issue where concurrent calls to `Cluster.Bucket` could race and result in multiple bucket agents being opened internally, which would leak even after `Cluster.Close` is called.

=== Version 2.12.0 (19 February 2026)

Version 2.12.0 is the first release in the Go SDK 2.12 series.

https://pkg.go.dev/github.com/couchbase/gocb/v2@v2.12.0?tab=doc[API Documentation]

Tracing and metrics with the `couchbase2` scheme should be considered stability level *volatile* at this time.

==== New Features and Behavioral Changes

* https://jira.issues.couchbase.com/browse/GOCBC-1769[GOCBC-1769]: Added support for mTLS certificate refresh without restart.
* https://jira.issues.couchbase.com/browse/GOCBC-1775[GOCBC-1775]: Deprecated support for MapReduce Views.
* https://jira.issues.couchbase.com/browse/GOCBC-1776[GOCBC-1776]: Added support for OpenTelemetry semantic conventions.


==== Fixed Issues

* https://jira.issues.couchbase.com/browse/GOCBC-1782[GOCBC-1782]: Fixed issue where specified scope was ignored when creating group.
it is now possible to specify role scope when creating a group.
// * https://jira.issues.couchbase.com/browse/GOCBC-1788[GOCBC-1788]: Fixed issue where app telemetry could panic when multiple nodes share an address (e.g. a load balanced environment).
...
=== Version 2.11.3 (18 February 2026)
...
=== Version 2.11.2 (21 January 2026)

Version 2.11.2 is a maintenance release for the Go SDK 2.11.

==== New Features and Behavioral Changes
* https://jira.issues.couchbase.com/browse/GOCBC-1780[GOCBC-1780]: Updated dependencies.

==== Fixed Issues

* https://jira.issues.couchbase.com/browse/GOCBC-1774[GOCBC-1774]: Fixed issue where get projections with > 16 would not correctly rebuild the projection.
* https://jira.issues.couchbase.com/browse/GOCBC-1778[GOCBC-1778]: Fixed issue where it was not able to reset durability level to none using `UpdateBucket`.
...
* https://jira.issues.couchbase.com/browse/GOCBC-1764[GOCBC-1764]: Non-TLS connections are not possible with the `couchbase2://` scheme. TLS is now always enabled when the `couchbase2://` scheme is used, and the system certificate pool is used by default if no certificate options are specified.
* https://jira.issues.couchbase.com/browse/GOCBC-1762[GOCBC-1762]: Improved logging for `WaitUntilReady` when no bootstrap error is found before a config is seen, to make it clearer that there was no error.
* https://jira.issues.couchbase.com/browse/GOCBC-1765[GOCBC-1765]: Improved error message when the `couchbase2://` scheme is used in a connection string with a bucket.

=== Version 2.11.0 (26 August 2025)

Version 2.11.0 is the first release in the Go SDK 2.11 series.

https://pkg.go.dev/github.com/couchbase/gocb/v2@v2.11.0?tab=doc[API Documentation]

Tracing and metrics with the `couchbase2` scheme should be considered stability level *volatile* at this time.

==== New Features and Behavioral Changes

* https://jira.issues.couchbase.com/browse/GOCBC-1633[GOCBC-1633], 
https://jira.issues.couchbase.com/browse/GOCBC-1739[GOCBC-1739]:
Added support for binary objects in transactions.
* https://jira.issues.couchbase.com/browse/GOCBC-1692[GOCBC-1692]:
Added missing fields to orphan reporter output.
* https://jira.issues.couchbase.com/browse/GOCBC-1693[GOCBC-1693]:
Updated app telemetry to contain latency for orphaned responses.
* https://jira.issues.couchbase.com/browse/GOCBC-1703[GOCBC-1703]:
Updated replica reads in transactions.
* https://jira.issues.couchbase.com/browse/GOCBC-1716[GOCBC-1716]:
Added support for FTS like pre-filters while doing vector search.
* https://jira.issues.couchbase.com/browse/GOCBC-1759[GOCBC-1759]:
Updated FTS `BooleanQuery` to accept a variadic parameter to allow a list of queries to be specified.

==== Fixed Issues

* https://jira.issues.couchbase.com/browse/GOCBC-1669[GOCBC-1669]:
Fixed issue where management operations had the incorrect metrics/span value for the `db.couchbase.service` attribute.
* https://jira.issues.couchbase.com/browse/GOCBC-1723[GOCBC-1723]:
Fixed issue where the incorrect error was returned for not locked in `couchbase2` mode.
* https://jira.issues.couchbase.com/browse/GOCBC-1730[GOCBC-1730]:
Fixed issue where `alt_node` was always reported for KV app telemetry.
* https://jira.issues.couchbase.com/browse/GOCBC-1742[GOCBC-1742]:
Fixed issue where `ErrDocumentExists` was wrapped in a `TransactionOperationFailedError` when it should not have been.
* https://jira.issues.couchbase.com/browse/GOCBC-1746[GOCBC-1746]:
Fixed issue where SRV record refresh would not correctly reset internal SDK state.


==== API breaking fixed Issues

* https://jira.issues.couchbase.com/browse/GOCBC-1758[GOCBC-1758]:
Fixed issue where FTS `TermRangeQuery` was requiring (and sending in the payload) a `term` attribute.
This led to the query always being executed as a `TermQuery`, and a `TermRangeQuery` never being executed.
...
* https://jira.issues.couchbase.com/browse/GOCBC-1724[GOCBC-1724]:
Fixed issue where `Append` and `Prepend` could panic in `couchbase2` mode if a retry occured.
* https://jira.issues.couchbase.com/browse/GOCBC-1735[GOCBC-1735]:
Fixed issue where Eventing function `QueryConsistency` was sent as the incorrect type on the wire.
* https://jira.issues.couchbase.com/browse/GOCBC-1737[GOCBC-1737]:
Fixed issue where reading search results could panic in `couchbase2` mode.
* https://jira.issues.couchbase.com/browse/GOCBC-1743[GOCBC-1743]:
Fixed issue where `GetMulti` read skew resolution could nil pointer.
* https://jira.issues.couchbase.com/browse/GOCBC-1745[GOCBC-1745]:
Fixed issue where SRV record refresh would never match existing addresses being used.
* https://jira.issues.couchbase.com/browse/GOCBC-1747[GOCBC-1747]:
Fixed issue where SRV record refresh would fail when cluster config pushes were active.
...
https://pkg.go.dev/github.com/couchbase/gocb/v2@v2.10.0?tab=doc[API Documentation]

Tracing and metrics with the `couchbase2` scheme should be considered stability level *volatile* at this time.

==== New Features and Behavioral Changes

* https://jira.issues.couchbase.com/browse/GOCBC-1278[GOCBC-1278]:
Added transactions support for `ExtReplaceBodyWithXattr`.
* https://jira.issues.couchbase.com/browse/GOCBC-1311[GOCBC-1311]:
Added transactions support for `ExtThreadSafety`.
* https://jira.issues.couchbase.com/browse/GOCBC-1430[GOCBC-1430]:
Updated path to `/whoami` rather than `/` for pinging the management service.
* https://jira.issues.couchbase.com/browse/GOCBC-1676[GOCBC-1676]:
Added support for application telemetry.
* https://jira.issues.couchbase.com/browse/GOCBC-1687[GOCBC-1687]:
Replaced usages of deprecated http transport dial functions.
* https://jira.issues.couchbase.com/browse/GOCBC-1688[GOCBC-1688]:
Replaced usages of deprecated `ioutil` functions.
* https://jira.issues.couchbase.com/browse/GOCBC-1701[GOCBC-1701]:
Added transactions support for `ExtGetMulti`.
* https://jira.issues.couchbase.com/browse/GOCBC-1704[GOCBC-1704]:
Added transactions support for specifying `NumVBuckets` in `BucketSettings`.
* https://jira.issues.couchbase.com/browse/GOCBC-1706[GOCBC-1706]:
Added support to the core for Columnar via a new Columnar Agent.
* https://jira.issues.couchbase.com/browse/GOCBC-1707[GOCBC-1707]:
Updated minimum compatible Go version to 1.21.
* https://jira.issues.couchbase.com/browse/GOCBC-1713[GOCBC-1713]:
Updated bucket management operations to convert HTTP status code 400 into `ErrInvalidArgument`.


==== Fixed Issues

* https://jira.issues.couchbase.com/browse/GOCBC-1648[GOCBC-1648]:
Fixed issue where `highlight.fields` would be sent in a search request payload even when the option wasn't specified.
* https://jira.issues.couchbase.com/browse/GOCBC-1712[GOCBC-1712]:
Fixed issue where `Append` and `Prepend` would return `ErrDocExists` rather than `ErrCasMismatch`.
* https://jira.issues.couchbase.com/browse/GOCBC-1715[GOCBC-1715]:
Fixed issue where `LoggingMeter` logged `emit_interval_s` as nanoseconds rather than seconds.
...
https://pkg.go.dev/github.com/couchbase/gocb/v2@v2.9.4?tab=doc[API Documentation]

Tracing and metrics with the `couchbase2` scheme should be considered stability level *volatile* at this time.

==== Fixed Issues

* https://jira.issues.couchbase.com/browse/GOCBC-1696[GOCBC-1696]:
Fixed an issue where scope-level {sqlpp} queries would fail if the scope or bucket names contained a period character (`.`).
...
Fixed a possible data race that occurred because the value of a lock was being logged at debug-level.
