...
+
TIP: xref:concept-docs:durability-replication-failure-considerations.adoc#synchronous-writes[Durable Write operations] have their own timeout setting, `KVDurabilityTimeout`, see below.

Name: *KVDurabilityTimeout*::
+
Default: `10s`
+
...
+
WARNING: The `KVDurabilityTimeout` property is not part of the stable API and may change or be removed at any time.
