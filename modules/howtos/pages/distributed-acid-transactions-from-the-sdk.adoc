...
The `Transactions` object is effectively a singleton belonging to a `Cluster` object. 
Once `Transactions` is created, its lifetime is bound to the parent `Cluster` object, this is because the `Transactions` object performs automated background processes that should not be duplicated.
