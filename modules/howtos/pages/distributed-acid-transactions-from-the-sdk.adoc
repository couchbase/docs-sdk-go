...
* Couchbase Go client 2.4.0 or above.
...
The `Transactions` object is effectively a singleton belonging to a `Cluster` object, once `Transactions` is created its lifetime is bound to the parent `Cluster` object.
This is because the `Transactions` object performs automated background processes that should not be duplicated.
...
The main different between `TransactionsQueryResult` and `QueryResult` is that `TransactionsQueryResult` does not stream results.
This means that there is no `Err` or `Close` functions and that result sets are buffered in memory - allowing the SDK to read and handle any errors that occur on the stream before returning a result/error.
...
=== Query Options
Query options can be provided via `TransactionsQueryOptions`, which provides a subset of the options in the Go SDK's `QueryOptions`.
...
IMPORTANT: Once one of these errors occurs, the current attempt is irrevocably failed (though the transaction may retry the lambda). It is not possible for the application to catch the error and continue. Once a failure has occurred, all other operations tried in this attempt (including commit) will instantly fail.
...
There are three errorz that the transactions library can return to the application: `TransactionFailedError`, `TransactionExpiredError` and `TransactionCommitAmbiguousError`.
