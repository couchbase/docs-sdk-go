...
Once a result returns you can iterate the returned rows and then access the `QueryMetaData` associated with the query (note that rows must be iterated before access to `QueryMetaData` is allowed).
...
From version 2.1.7 of the Go SDK, it is possible to query off the xref:7.0@server:learn:data/scopes-and-collections.adoc[`Scope` level] _with the beta version of the next Couchbase Server release, 7.0β_,
...
query := fmt.Sprintf("SELECT x.* FROM %s x LIMIT 10;", myCollection)
        rows, err := us.Query(query, &gocb.QueryOptions{})
