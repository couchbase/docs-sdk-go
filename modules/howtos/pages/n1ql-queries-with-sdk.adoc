...
the assumption is that the `airline` collection exists within a scope `myScope`.

[source,golang]
----
myScope := cluster.Bucket("mybucket").Scope("myscope")
rows, err := myScope.Query("SELECT x.* FROM `airline` x LIMIT 10;", &gocb.QueryOptions{})
