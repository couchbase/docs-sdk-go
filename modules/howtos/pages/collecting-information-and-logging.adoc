...
Once the SDK writes the logs with the tags to a file, you can then use the xref:{version-server}@server:cli:cbcli/cblogredaction.adoc[`cblogredaction` tool] to obfuscate the log.

* You may wish to read more on Log Redaction xref:{version-server}@server:manage:manage-logging/manage-logging.adoc#understanding_redaction[in the Server docs].
...
[source,golang,indent=0]
----
include::devguide:example$go/custom-logging_test.go[tag=loggerwrapper]
----
...
[source,golang,indent=0]
----
include::devguide:example$go/custom-logging_test.go[tag=creation]
----
