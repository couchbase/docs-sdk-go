...
If you wish to create an array if it does not exist and also push elements to it within the same operation you may use the <<subdoc_create_path,_CreateParents_>> option:

.Array appending a sub-document with create path
[source,golang,indent=0]
----
include::example$subdoc.go[tag=mutateInArrayAppendCreateParents]
----
...
If the value does not exist, the operation will create it (and its parents, if _CreateParents_ is enabled).
...
In this case, the _CreateParents_ option may be used.

.Upsert with CreateParents
[source,golang,indent=0]
----
include::example$subdoc.go[tag=mutateInCreateParents]
----
...
So in some cases the application will not need to supply a xref:concurrent-mutations-cluster.adoc[CAS] value to protect against concurrent modifications.
