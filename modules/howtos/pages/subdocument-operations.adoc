...
* Sub-Document counter operations can operate on any numeric value within a document, while xref:howtos:kv-operations.adoc#atomic-counters[full-document counter operations] require a specially formatted counter document with only the counter value.
