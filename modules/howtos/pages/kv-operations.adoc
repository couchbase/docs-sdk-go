...
WARNING: These pages cover the first _beta_ of the Couchbase Go SDK -- 2.0.
...
By using the CAS value we know that no-one else can have modified this document and updated it before our update.
...
The older type of durability, observe based durability, works by monitoring the server to ensure that the change has been replicated or persisted to the required number of nodes within
the timeout specified on the operation. Here we can see how that is set:
