...
This can be used to control whether the SDK connects
...
Use multiple Data nodes in your connection string to to improve bootstrap resilience. 
...
// Use multiple Data nodes for bootstrap resilience
cluster, err := gocb.Connect("data-node1.example.com,data-node2.example.com,data-node3.example.com", opts)
----

NOTE: This requirement applies to all Couchbase SDKs and products that use connection strings, including Sync Gateway.
