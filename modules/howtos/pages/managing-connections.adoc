...
All high-level objects in the Go SDK are designed to be safe for concurrent use by multiple goroutines.  You will get the best performance by
using only a single Cluster object per cluster, and as few `Bucket`, `Scope` and `Collection` as is reasonable for your application.
