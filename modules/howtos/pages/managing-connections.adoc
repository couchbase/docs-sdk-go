...
Couchbase's next generation connection protocol, introduced in Go SDK 2.7 and Couchbase Autonomous Operator 2.6, can be enabled simply by changing the connection string to `couchbase2://` but there are a few differences to be aware of, described <<#limitations,below>>.

The protocol implements a gRPC-style interface between the SDK and Couchbase Server (in this case, only available in the Server running on Kubernetes, with xref:operator::overview.adoc[Couchbase Autonomous Operator].



=== Limitations

The protostellar protocol will not work with certain legacy features: MapReduce Views, Memcached buckets.
...
* Some config options are unsupported -- see the xref:ref:client-settings.adoc[Settings page].
...
* Some error codes are more generic -- in cases where the client would not be expected to need to take specific action.
