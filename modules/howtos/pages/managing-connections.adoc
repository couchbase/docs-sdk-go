...
The protocol implements a gRPC-style interface between the SDK and Couchbase Server (in this case, only available in the Server running on Kubernetes or OpenShift, with xref:operator::overview.adoc[Couchbase Autonomous Operator].
...
* Some config options are unsupported -- see the xref:ref:client-settings.adoc#cloud-native-gatewry[Settings page].
